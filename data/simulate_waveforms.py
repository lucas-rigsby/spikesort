...
# ── RC time constants — must match breadboard component values exactly ─────────
...
def extract_features(waveform, threshold=THRESHOLD):
    ...
    # F2 — peak timing (valid: uses amplitude to find true maximum)
    F2 = float(peak_idx) / WINDOW_SIZE

    # F3 — rise ratio (valid: amplitude allows pre/post-peak distinction)
...
if __name__ == "__main__":
    ...
    # F1 separation ratio (key discriminator)
